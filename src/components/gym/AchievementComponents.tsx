...
import { UserStats, ACHIEVEMENTS } from "@/utils/achievementSystem";
...
> = ({ achievements, stats }) => {
  ...
  const recentAchievements = [...achievements]
    .sort((a, b) => {
      const dateA = new Date(a.unlockedAt || a.date || 0).getTime();
      const dateB = new Date(b.unlockedAt || b.date || 0).getTime();
      return dateB - dateA;
    })
    .slice(0, 3);
  ...
  const completionPercentage = Math.round(
    (achievements.length / ACHIEVEMENTS.length) * 100
  );
