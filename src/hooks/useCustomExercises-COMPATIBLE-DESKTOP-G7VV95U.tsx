...
export function useCustomExercises() {
  ...
  const fetchExercises = useCallback(async () => {
    try {
      ...
      if (
        ...
        console.log(
          "Skipping custom exercises load - auth session not ready yet"
        );
        setError(null);
      } else {
        setError(message);
  ...
  useEffect(() => {
    ...
    const subscription = realtimeAPI.subscribeToCustomExercises(
      (payload: unknown) => {
        ...
        if (typedPayload.eventType === "INSERT") {
          setExercises((prev) => [typedPayload.new as CustomExercise, ...prev]);
        } else if (typedPayload.eventType === "UPDATE") {
          setExercises((prev) =>
            prev.map((ex) =>
              ex.id === typedPayload.new.id
                ? (typedPayload.new as CustomExercise)
                : ex
            )
          );
        } else if (typedPayload.eventType === "DELETE") {
          setExercises((prev) =>
            prev.filter((ex) => ex.id !== typedPayload.old.id)
          );
        }
      }
    );
