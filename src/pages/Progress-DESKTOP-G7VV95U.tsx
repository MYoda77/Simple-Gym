...
const Progress = () => {
    ...
    if (activities) {
      try {
        ...
        if (weightActivities.length > 0) {
          ...
          weightActivities.forEach((activity: Activity) => {
  ...
  const handleLogWeight = (data: {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        workoutHistory.forEach((workout: WorkoutRecord) => {
  ...
  // Update workout stats from workout history
  useEffect(() => {
    const totalWorkouts = workoutHistory.length;
    const thisWeekWorkouts = getThisWeekWorkouts(workoutHistory);
    const streakData = calculateStreak(workoutHistory);
    const currentStreak = streakData.currentStreak;

    setStats((prev) => {
      const currentWeight = prev.find((s) => s.label === "Current Weight");
      const currentBMI = prev.find((s) => s.label === "Current BMI");
      ...
      return [
        ...
        { label: "Streak", value: currentStreak, icon: "🔥", unit: "days" },
      ];
    });
  }, [workoutHistory]);
  ...
  const handleResetClick = () => {
    // Redirect to Profile page where the comprehensive Reset All Data is located
    navigate("/profile");
  };

  return (
    <div className="min-h-screen bg-background">
      <ProgressHeader onBack={() => navigate("/")} onReset={handleResetClick} />
        ...
        <div className="px-2 sm:px-4 py-4 space-y-6">
          ...
          <ProgressiveOverload
            ...
            onStartWorkout={(workoutName) => {
              navigate("/", { state: { startWorkout: workoutName } });
            }}
          />
