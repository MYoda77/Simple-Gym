...
export class WorkoutRecommendationEngine {
  ...
  generateRecommendations(): WorkoutRecommendation[] {
    ...
    recommendations.push(
      ...this.generateMuscleBalanceRecommendations(analysis)
    );
  ...
  private analyzeTraining(): TrainingAnalysis {
    ...
    // Recent workouts
    const lastWeekWorkouts = this.workoutHistory.filter(
      (w) => new Date(w.date) >= oneWeekAgo
    );
    const lastMonthWorkouts = this.workoutHistory.filter(
      (w) => new Date(w.date) >= oneMonthAgo
    );

    // Volume calculations
    const totalSetsLastWeek = lastWeekWorkouts.reduce(
      (sum, w) => sum + (w.totalSets || 0),
      0
    );
    const totalSetsLastMonth = lastMonthWorkouts.reduce(
      (sum, w) => sum + (w.totalSets || 0),
      0
    );
    ...
    lastMonthWorkouts.forEach((workout) => {
      const exercise = this.exerciseDatabase.find(
        (e) => e.name === workout.name
      );
    ...
    // Find imbalances
    const avgCount =
      Object.values(muscleGroupCounts).reduce((a, b) => a + b, 0) /
    ...
    // Recovery metrics
    const sortedWorkouts = [...this.workoutHistory].sort(
      (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
    );
    const daysSinceLastWorkout =
      sortedWorkouts.length > 0
        ? Math.floor(
            (now.getTime() - new Date(sortedWorkouts[0].date).getTime()) /
              (1000 * 60 * 60 * 24)
          )
        : 999;
  ...
  private generateNextWorkoutRecommendations(
    ...
    if (
      analysis.daysSinceLastWorkout <= 2 &&
      analysis.undertrainedMuscles.length > 0
    ) {
      ...
      recommendations.push({
        ...
        reasoning: [
          `${targetMuscle} trained only ${
            analysis.muscleGroupDistribution[targetMuscle] || 0
          } times this month`,
    ...
    if (preferredDay && analysis.daysSinceLastWorkout === 0) {
      // Find the most trained muscle group to suggest
      const topMuscleGroups = Object.entries(analysis.muscleGroupDistribution)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 2)
        .map(([muscle]) => muscle);

      const suggestedWorkout =
        topMuscleGroups.length > 0 ? topMuscleGroups.join(" + ") : "Full Body";

      const suggestedExercises =
        topMuscleGroups.length > 0
          ? this.findExercisesForMuscle(topMuscleGroups[0]).slice(0, 3)
          : [];

      const exerciseList =
        suggestedExercises.length > 0
          ? `Try: ${suggestedExercises.join(", ")}`
      ...
      recommendations.push({
        ...
        description: `You typically train on ${preferredDay}s. Suggested: ${suggestedWorkout} workout. ${exerciseList}`,
        reasoning: [
        ...
        action: {
          ...
          details: {
            ...
            suggestedMuscles: topMuscleGroups,
            suggestedExercises,
          },
        },
        confidence: 75,
      ...
      analysis.undertrainedMuscles.forEach((muscle, idx) => {
        if (idx < 2) {
          ...
          recommendations.push({
            ...
            reasoning: [
              `${muscle} trained ${
                analysis.muscleGroupDistribution[muscle] || 0
              }x vs average ${Math.round(
                Object.values(analysis.muscleGroupDistribution).reduce(
                  (a, b) => a + b,
                  0
                ) / Object.keys(analysis.muscleGroupDistribution).length
              )}x`,
  ...
  private generateRecoveryRecommendations(
    ...
    if (analysis.restDaysLastWeek === 0 && analysis.workoutsLastWeek >= 5) {
      recommendations.push({
        ...
        description:
          "You had no rest days last week. Plan at least 1-2 rest days per week.",
        reasoning: [
    ...
    if (
      analysis.daysSinceLastWorkout >= 4 &&
      analysis.daysSinceLastWorkout < 14
    ) {
      recommendations.push({
  ...
  private generateProgressionRecommendations(
    ...
    if (
      ...
      recommendations.push({
        ...
        description:
          "You haven't set a PR in a while. Try these strategies to break through.",
        reasoning: [
  ...
  private generateVarietyRecommendations(
    ...
    if (analysis.leastRecentExercises.length > 0) {
      ...
      recommendations.push({
        ...
        description:
          "You haven't done this exercise in a while. Consider adding it back.",
        reasoning: [
  ...
  private generateVolumeRecommendations(
    ...
    if (analysis.avgWorkoutsPerWeek < 2 && this.workoutHistory.length > 4) {
      recommendations.push({
        ...
        description: `You're averaging ${analysis.avgWorkoutsPerWeek.toFixed(
          1
        )} workouts per week. Aim for 3-4.`,
        reasoning: [
    ...
    if (analysis.avgWorkoutsPerWeek > 6) {
      recommendations.push({
        ...
        description: `You're training ${analysis.avgWorkoutsPerWeek.toFixed(
          1
        )} times per week. Make sure you're recovering.`,
        reasoning: [
  ...
  private generateDeloadRecommendations(
    ...
    if (needsDeload) {
      recommendations.push({
        ...
        description:
          "You've been training hard. Take a deload week to supercompensate.",
        reasoning: [
