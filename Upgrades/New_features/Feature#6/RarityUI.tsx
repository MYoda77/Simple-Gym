...
export const TierBadge: React.FC<TierBadgeProps> = ({ tier, size = 'md' }) => {
  ...
  return (
    <Badge
      className={`bg-gradient-to-r ${config.color} text-white border-2 ${config.borderColor} ${sizeClasses[size]} font-bold ${config.special || ''}`}
    >
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-4 gap-2 pt-4 border-t border-border">
          {(['bronze', 'silver', 'gold', 'platinum'] as AchievementTier[]).map(
            (tier) => {
              const isUnlocked = totalPoints >= TIER_CONFIG[tier] || tier === tierProgress.currentTier;
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs font-medium">{TIER_CONFIG[tier].name}</p>
                </div>
              );
            }
          )}
 ...
 */
export const rarityAnimationStyles = `
