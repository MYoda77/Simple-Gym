...
export function calculateRarityCompletion(
  achievements: Achievement[]
): Record<AchievementRarity, { unlocked: number; total: number; percentage: number }> {
  const totals = {
    common: ACHIEVEMENT_RARITY_DEFINITIONS.filter((d) => d.rarity === 'common').length,
    rare: ACHIEVEMENT_RARITY_DEFINITIONS.filter((d) => d.rarity === 'rare').length,
    epic: ACHIEVEMENT_RARITY_DEFINITIONS.filter((d) => d.rarity === 'epic').length,
    legendary: ACHIEVEMENT_RARITY_DEFINITIONS.filter((d) => d.rarity === 'legendary').length,
  };
  ...
  return {
    common: {
      ...
      total: totals.common,
      percentage: totals.common > 0 ? Math.round((distribution.common / totals.common) * 100) : 0,
    },
    rare: {
      unlocked: distribution.rare,
      total: totals.rare,
      percentage: totals.rare > 0 ? Math.round((distribution.rare / totals.rare) * 100) : 0,
    },
    epic: {
      unlocked: distribution.epic,
      total: totals.epic,
      percentage: totals.epic > 0 ? Math.round((distribution.epic / totals.epic) * 100) : 0,
    },
    legendary: {
      unlocked: distribution.legendary,
      total: totals.legendary,
      percentage: totals.legendary > 0 ? Math.round((distribution.legendary / totals.legendary) * 100) : 0,
    },
  };
}
